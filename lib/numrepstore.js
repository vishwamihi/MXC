...
let msg = [];
let databaseName = 'sachibot_movie_data'
...
const add_old_data_to_msg_array_retrive_from_mongodb_when_start = async () => {
  try {
    if (client === null) {
      ...
      console.log('Formatted data stored in msg:', msg);
      return true;
    }
  } catch (error) {
  ...
  const upload_to_mongodb = async (data,databaseName,collectionName) => {
      try {
        if(client ===null){
          ...
          console.log('Data uploaded to MongoDB:', data);
          return true;
        }
      } catch (error) {
  ...
  const storenumrepdata = async (json) => {
      try {
        if(client ===null){
          ...
          console.log('Data stored in MongoDB:', json);
          console.log('data set of msg array: ', msg)
